...
global.currencyName = 'Electerizer';
global.currencyPlural = 'Electerizers';
let shop = [
...
function getShopDisplay(shop) {
	let display = `<center><img src=https://play.pokemonshowdown.com/sprites/xyani/electivire.gif><img src=https://images.cooltext.com/5284976.png width=300> <img src=https://play.pokemonshowdown.com/sprites/xyani/electivire.gif></center><br /><div class="infobox-limited"><table style="background: #2ae10e; border-color: #2e6dd8; border-radius: 8px" border="1" cellspacing="0" cellpadding="5" width="100%"><tbody><tr><th><font color=#2e6dd8 face=courier>Item</font></th><th><font color=#2e6dd8 face=courier>Description</font></th><th><font color=#2e6dd8 face=courier>Price</font></th></tr>`;
	display += `<marquee style="background: #0000ff ; border: 10px solid red ; border-radius: 10px"><psicon pokemon="kyurem-black"><psicon pokemon="kyurem-black"</marquee>`
  let start = 0;
...
global.rankLadder = function (title, type, array, prop, group) {
	...
	for (let i = 0; i < array.length; i++) {
		if (i === 0) {
			...
			tableRows += '<tr><td ' + first + '>' + (i + 1) + midColumn + FTN.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		} else if (i === 1) {
			midColumn = '</td><td ' + second + '>';
			tableRows += '<tr><td ' + second + '>' + (i + 1) + midColumn + FTN.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		} else if (i === 2) {
			midColumn = '</td><td ' + third + '>';
			tableRows += '<tr><td ' + third + '>' + (i + 1) + midColumn + FTN.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		} else {
			midColumn = '</td><td ' + tdStyle + '>';
			tableRows += '<tr><td ' + tdStyle + '>' + (i + 1) + midColumn + FTN.nameColor(array[i].name, true) + midColumn + array[i][prop] + '</td></tr>';
		}
	}
...
exports.commands = {
  ...
  store: "shop",
	...
	shophelp: [`/shop - Display items you can buy with your ${currencyPlural}.`],
  '!wallet': true,
	atm: 'wallet',
	wallet: function (target, room, user) {
		...
		Economy.readMoney(userid, money => {
			this.sendReplyBox(FTN.nameColor(target, true) + " has " + money + ((money === 1) ? " " + currencyName + "." : " " + currencyPlural + "."));
			//if (this.broadcasting) room.update();
		});
	},

	gs: 'givecurrency', //You can change "gs" and "giveBucks" to your currency name for an alias that applies to your currency Example: AwesomeBucks could be "ga" and "giveawesomebucks"
		...
		Economy.writeMoney(targetUser, amount, () => {
			Economy.readMoney(targetUser, newAmount => {
				if (Users(targetUser) && Users(targetUser).connected) {
					...
					' from ' + FTN.nameColor(user.userid, true) + '.');
				}
	...
	ts: 'takecurrency', //You can change "ts" and "takeBucks" to your currency name for an alias that applies to your currency Example: AwesomeBucks could be "ta" and "takeawesomebucks"
	...
	takecurrency: function (target, room, user, connection, cmd) {
		...
		Economy.writeMoney(targetUser, -amount, () => {
			Economy.readMoney(targetUser, newAmount => {
				if (Users(targetUser) && Users(targetUser).connected) {
					Users.get(targetUser).popup('|html|' + FTN.nameColor(user.userid, true) + ' has removed ' + amount + ' ' + (amount === 1 ? currencyName : currencyPlural) +
					' from you.<br />');
				}
		...
		Economy.readMoney(user.userid, money => {
			...
			if (cmd !== 'confirmtransfercurrency' && cmd !== 'confirmtransferBucks') {
				return this.popupReply('|html|<center>' +
					...
					'Confirm transfer to <br><b style = "color:' + FTN.hashColor(targetUser) + '; text-shadow: 1px 1px 1px rgba(0, 0, 0, 0.8)">' + Chat.escapeHTML(targetUser) + '</b></button></center>'
				);
			}
					...
					Economy.readMoney(targetUser, firstAmount => {
						Economy.readMoney(user.userid, secondAmount => {
							...
							if (Users.getExact(targetUser) && Users.getExact(targetUser).connected) {
								Users.getExact(targetUser).send('|popup||html|' + FTN.nameColor(user.name, true) + " has sent you " + amount + ((amount === 1) ? " " + currencyPlural : " " + currencyPlural));
							}
						});
					});
				});
